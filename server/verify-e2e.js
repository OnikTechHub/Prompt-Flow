...
async function runTests() {
  ...
  await User.deleteMany({ 
    email: { 
      $in: [
        "test-verify-e2e@prompt-flow.ai", 
        "demo@prompt-flow.ai"
      ] 
    } 
  });
  ...
  const registerReq = {
    body: {
      name: "Test E2E User",
      email: "test-verify-e2e@prompt-flow.ai",
  ...
  if (demoRes.statusCode === 200 && demoRes.jsonData.success) {
    ...
    const demoUser = await User.findOne({ email: "demo@prompt-flow.ai" });
  ...
  const finalDemoUser = await User.findOne({ email: "demo@prompt-flow.ai" });
  if (finalDemoUser) {
