...
export default function Pricing() {
  ...
  const plans = [
    ...
    },
    {
      ...
      cta: "Upgrade to Pro",
      popular: true,
      accent: "border-cyan-500/50 bg-gradient-to-br from-[#0c0824] via-indigo-950/20 to-[#030014] shadow-lg shadow-cyan-500/5"
    },
    {
      name: "Enterprise",
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto">
          ...
          <div className="mt-8 flex justify-center items-center gap-3">
            ...
            >
              <div className={`h-4.5 w-4.5 bg-teal-500 rounded-full transition-transform duration-300 ${
        ...
        <div className="mt-16 grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch max-w-6xl mx-auto">
          {plans.map((plan, idx) => {
            ...
            return (
              ...
              >
                {plan.popular && (
                  <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 inline-flex items-center gap-1 px-3 py-1 rounded-full border border-cyan-400 bg-teal-600 text-[10px] font-bold uppercase tracking-wider text-white shadow-lg shadow-cyan-500/25">
                  ...
                  <ul className="space-y-3.5">
                    {plan.features.map((feat, i) => (
                      <li key={i} className="flex items-start gap-2.5 text-xs text-slate-300">
                        <Check className="h-4 w-4 text-teal-400 mt-0.5 flex-shrink-0 bg-teal-500/10 p-0.5 rounded-full" />
                        <span>{feat}</span>
                      </li>
                    ))}
                  </ul>
                ...
                <button
                  className={`mt-8 w-full py-3 px-4 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                    plan.popular
                      ? "bg-gradient-to-r from-teal-600 to-cyan-500 hover:shadow-lg hover:shadow-cyan-500/25 text-white hover:scale-[1.01] active:scale-[0.99]"
