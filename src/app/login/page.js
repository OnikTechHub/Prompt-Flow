...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (token) {
        localStorage.setItem("prompt-flow_jwt", token);
  ...
  const handleDemoLogin = async (e) => {
    ...
    setError("");
    try {
      await login("demo@prompt-flow.ai", "demopass123");
      setSuccess(true);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#030014] text-slate-100 flex flex-col items-center justify-center relative px-4 py-12">
      ...
      <div className="absolute inset-0 bg-[linear-gradient(to_right,rgba(255,255,255,0.01)_1px,transparent_1px),linear-gradient(to_bottom,rgba(255,255,255,0.01)_1px,transparent_1px)] bg-[size:4rem_4rem] opacity-25 pointer-events-none" />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-[500px] w-[500px] rounded-full bg-cyan-950/10 blur-[100px] pointer-events-none" />
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-tr from-teal-600 to-cyan-500 text-white shadow-lg shadow-cyan-500/20 mb-4">
        ...
        <div className="glass-panel rounded-2xl p-6 sm:p-8 border border-white/10 shadow-2xl shadow-cyan-950/15 animate-fade-in">
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="mt-1.5 relative">
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 rounded-xl bg-black/40 border border-white/5 focus:border-cyan-500/50 focus:outline-none text-sm text-white placeholder-slate-600 transition-colors disabled:opacity-50"
                />
            ...
            <div>
              <div className="flex justify-between items-center">
                ...
                <a href="#forgot" onClick={(e) => e.preventDefault()} className="text-[10px] text-cyan-400 hover:underline">Forgot password?</a>
              </div>
              <div className="mt-1.5 relative">
                <input
                  ...
                  placeholder="••••••••"
                  disabled={loading || success}
                  className="w-full pl-10 pr-10 py-3 rounded-xl bg-black/40 border border-white/5 focus:border-cyan-500/50 focus:outline-none text-sm text-white placeholder-slate-600 transition-colors disabled:opacity-50"
                />
            ...
            <button
              type="submit"
              disabled={loading || success}
              className="w-full py-3 rounded-xl bg-gradient-to-r from-teal-600 to-cyan-500 hover:shadow-lg hover:shadow-cyan-500/25 text-white text-sm font-bold transition-all duration-300 hover:scale-[1.01] active:scale-[0.99] disabled:opacity-50 disabled:scale-100 flex items-center justify-center gap-2 cursor-pointer animate-pulse-glow"
            >
              {loading ? (
                <>
          ...
          <button
            ...
            disabled={loading || success}
            className="w-full py-3.5 rounded-xl border border-dashed border-cyan-500/30 bg-teal-500/5 hover:bg-teal-500/10 text-cyan-300 hover:text-cyan-200 text-xs font-bold transition-all duration-300 hover:scale-[1.01] active:scale-[0.99] disabled:opacity-50 disabled:scale-100 flex items-center justify-center gap-2 cursor-pointer mt-3"
          >
